...
Migration Phase 3 Update:
  ...
  - Path resolution via utils.resolve_w2_paths()
...
Spec: v5.4.2-P9.2 + Migration Phase 2 v0.2.1 + Migration Phase 3 v0.3.1
"""
...
DEFAULT_W2_BASE_DIR = "data/stats"
...
class W2Aggregator:
    ...
    Migration Phase 3 Usage:
        ...
        # Files saved to: data/stats/policies/{policy_id}/{version}/{scope}/
    ...
    def __init__(
        ...
        base_dir: str = DEFAULT_W2_BASE_DIR,
    ):
        """
        Initialize W2 Aggregator.
        
        Args:
            records_path: Path to W2 records JSONL (overrides path resolution)
            conditions_path: Path to conditions registry JSONL (overrides path resolution)
        ...
        # Resolve paths using Migration Phase 3 utils
        resolved_paths = self._resolve_paths(records_path, conditions_path)
        self.records_path = resolved_paths['records_path']
        self.conditions_path = resolved_paths['conditions_path']
    ...
    def _resolve_paths(
        ...
        conditions_path: Optional[str],
    ) -> Dict[str, str]:
        """
        Resolve file paths for W2 data.
        
        Migration Phase 3: Uses utils.resolve_w2_paths() for Policy mode.
        
        Priority:
          1. Explicit paths (if provided)
          2. Path resolution (Policy mode → isolated directory)
          3. Default paths (Legacy mode)
        
        Args:
            records_path: Explicit records path (or None)
            conditions_path: Explicit conditions path (or None)
        ...
        """
        # If both paths are explicitly provided, use them directly
        if records_path and conditions_path:
            return {
        ...
        from .utils import resolve_w2_paths, ScopeValidationError
        
        # Use path resolver (validates scope in Policy mode)
        try:
            resolved = resolve_w2_paths(
                self.base_dir,
                self.policy,
            ...
            raise ScopeValidationError(
                f"W2Aggregator requires analysis_scope_id when policy is set. "
        ...
        return {
            'records_path': records_path or resolved['records_path'],
            'conditions_path': conditions_path or resolved['conditions_path'],
        }
    ...
    def _get_condition_signature(self, article) -> str:
        ...
        Returns:
            Condition signature (64 hex chars for Policy, 64 hex for Legacy)
        """
