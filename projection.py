...
class AtomFieldEmbeddings:
    ...
    def _cache_path(self) -> Path:
        return self.cache_dir / f"atom_def_emb_{self.embedder.name}.npz"
    
    def _meta_path(self) -> Path:
        return self.cache_dir / f"atom_def_emb_{self.embedder.name}_meta.json"
    ...
    def _compute(self, dict_hash: str):
        ...
        for atom_id in sorted(concepts.keys()):
            ...
            # Build text: "atom_id. label. definition"
            parts = [atom_id]
...
class HybridProjection:
    ...
    def project(self, synapse_candidates: List[Dict],
        ...
        """
        Project Synapse candidates through field gate + Bayesian update.
        
        Args:
            synapse_candidates: [{"atom": "STA.wealth", "score": 0.72}, ...]
            sentence_emb: (dim,) sentence embedding
            atom_field: precomputed atom embeddings
            top_k: number of results to return
        
        Returns:
            projected_topk: [{"atom": ..., "score": ..., "prior": ..., "gate": ...}]
            stats: gate_stats + projection metadata
        """
        # Build prior from Synapse candidates
        prior = build_prior_from_synapse_candidates(synapse_candidates)
...
class FieldFirstProjection:
    ...
    def project(self, synapse_candidates: List[Dict],
        ...
        results = []
        for cand in synapse_candidates:
